...
#if !defined(USE_SERIAL) && !defined(USE_OMP) && !defined(USE_MPI) && !defined(USE_MPI_OMP) && !defined(USE_KOKKOS) && \
    !defined(USE_CUDA) && !defined(USE_HIP)
...
template <typename DType> void copyVectorToView(std::vector<DType> const &vec, Kokkos::View<DType *> view) {
...
template <typename DType> void copyViewToVector(Kokkos::View<DType *> view, std::vector<DType> &vec) {
  assert(vec.size() == view.size());
  for (int i = 0; i < vec.size(); i += 1) {
    vec[i] = view(i);
  }
...
template <typename DType> void fillRandKokkos(Kokkos::View<DType *> &x, DType min, DType max) {
...
template <typename T> void fillRandString(T &x, size_t minLen, size_t maxLen) {
  std::string tmp = "xxxxxx";
  std::fill(x.begin(), x.end(), tmp);
}

// utility functions
// T like std::vector<DType>
template <typename T, typename DType> void fillRand(T &x, DType min, DType max) { std::fill(x.begin(), x.end(), 0.0); }

// compare two vectors of floating point numbers
template <typename Vec, typename FType> bool fequal(Vec const &a, Vec const &b, FType epsilon = 1e-6) {
  assert(a.size() == b.size());
  return std::equal(a.begin(), a.end(), b.begin(), [epsilon](FType x, FType y) { return std::abs(x - y) <= epsilon; });
}
